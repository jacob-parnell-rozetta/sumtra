...
class LmForSummarization(pl.LightningModule):
    ...
    def forward(self, src_ids, tgt_ids, src_attention_mask, tgt_attention_mask, step_type):
    ...
    def validation_step(self, batch, batch_nb, step_type='val'):
        ...
        vloss = outputs[0]
        # Generate summary
        src_ids, tgt_ids, src_attention_mask, tgt_attention_mask = batch
        ...
        if isinstance(self.tokenizer, MBartTokenizer) or isinstance(self.tokenizer, MBart50Tokenizer):
            ...
            generated_ids = self.model.generate(input_ids=src_ids, attention_mask=src_attention_mask,
                                                decoder_start_token_id=get_lang_id(LANGUAGE_CODE_MAPPING[self.args['tgt']]['name']),
                                                max_length=self.args['max_output_len'],
                                                num_beams=1, num_beam_groups=1, do_sample=False)
    ...
    def validation_epoch_end(self, outputs, step_type='val'):
        ...
        # Save language model when validating only
        if step_type == 'val':
            self.save_language_model(logs['vloss'])

        return {'avg_val_loss': logs['vloss'], 'log': logs, 'progress_bar': logs}

    def save_language_model(self, r_score):
        r_score = round(r_score.detach().tolist(), 4)
        path_save = os.path.join(self.args['save_dir'],
                                 self.args['save_prefix'], f"checkpoints_ep{self.current_epoch}_" + str(r_score))
...
def main(args):
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        save_top_k=1,
        verbose=True,
        monitor='validation_loss',  # monitor for highest rouge-2
        mode='min',
        every_n_epochs=1
    )
    progress_bar = pl.callbacks.TQDMProgressBar(refresh_rate=10)
    early_stopping = EarlyStopping(monitor='validation_loss', patience=2,
    ...
    trainer = pl.Trainer(gpus=args.gpus, accelerator=None,
                         ...
                         num_sanity_val_steps=0,  # TODO: note that this runs a save ckpt but will simulate zero-shot
    ...
    if not args.test:
        ...
        print(file_list)
        # Check which one has the lowest score (we are running val loss)
        best_number = 1000
        best_filename = None
        for filename in file_list:
            if filename.startswith('checkpoint') and filename != 'checkpoints':  # skips the instance saved by model ckpt
                number = float(filename.split('_')[2])  # gets the vloss value
        ...
        # Copy the pre-trained tokenizer into this save path
        shutil.copy('pretrained_lm/facebook-mbart-large-50-many-to-one-mmt/tokenizer.json',
                    f'{path_save}/{best_filename}')
    trainer.test(model)
