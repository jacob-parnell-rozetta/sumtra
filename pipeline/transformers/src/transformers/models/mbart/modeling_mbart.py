...
def shift_token_embeds_right(inputs_embeds: torch.Tensor, attention_mask: torch.Tensor, pad_token_embed: torch.tensor,
                             decoder_start_token_embed: torch.tensor):
    """
    Shift input ids one token to the right.
    """
    # shifted_inputs_embeds = inputs_embeds.new_zeros(inputs_embeds.shape)
    bsz, _, _ = inputs_embeds.size()
    shifted_inputs_embeds = inputs_embeds[:, :-1, :]
    decoder_embed = decoder_start_token_embed.unsqueeze(0).unsqueeze(0).repeat(bsz, 1, 1)
    shifted_inputs_embeds = torch.cat((decoder_embed, shifted_inputs_embeds), dim=1)

    assert pad_token_embed is not None, "PAD token embedding has to be defined."
    # replace possible -100 values in labels by `pad_token_id`

    shifted_inputs_embeds[attention_mask == 0] = pad_token_embed

    return shifted_inputs_embeds
...
class MBartModel(MBartPreTrainedModel):
    ...
    def forward(
        ...
        if decoder_input_ids is None and inputs_embeds is None:
            decoder_input_ids = shift_tokens_right(
                input_ids, self.config.pad_token_id, bos_token_id
            )
        elif decoder_inputs_embeds is None and input_ids is None and encoder_outputs is None:
            decoder_inputs_embeds = shift_token_embeds_right(
                inputs_embeds, attention_mask=attention_mask, pad_token_embed=pad_token_embed,
                decoder_start_token_embed=bos_token_embed
            )
...
class MBartForConditionalGeneration(MBartPreTrainedModel):
    ...
    def forward(
        ...
        output_attentions=None,
        output_hidden_states=None,
        pad_token_embed=None,  # added to be passed to MBartModel()
        bos_token_embed=None,  # added to be passed to MBartModel()
        bos_token_id=None,  # added to be passed to MBartModel()
        return_dict=None
    ):
        r"""
        ...
        outputs = self.model(
            ...
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
            pad_token_embed=pad_token_embed,  # Added to facilitate e_embs()
            bos_token_embed=bos_token_embed,  # Added to facilitate e_embs()
            bos_token_id=bos_token_id  # Added to facilitate e_embs()
        )
...
class MBartForSequenceClassification(MBartPreTrainedModel):
    ...
    def forward(
        ...
        # print(torch.sum(hidden_states))  # To help debuging

        if pred_ids is not None:
            eos_mask = pred_ids.eq(self.config.eos_token_id)
        else:
